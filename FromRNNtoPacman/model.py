...
class NeuralNetworkModel():
    ...
    def forward(self, X):
        ...
        """
        ### YOUR CODE HERE ###
        for layer in self.layers: # looping through all layers
          X = layer.forward(X) # doing a pass forward for each layer 
        return X # returning the final forward pass output of last layer
    ...
    def backward(self, target, out):
        ...
        """
        ### YOUR CODE HERE ###
        loss = self.loss.forward(target, out) #         # Compute the loss
        # Backpropagate through the network's layers.
        gradient= self.loss.backward(target, out) # compute grad of loss w.r.t to putput
        for layer in reversed(self.layers): # looping through all layers
          gradient = layer.backward(gradient) # doing a backward pass for each layer
        return loss # return computed final loss
    ...
    def predict(self, X, Y):
        ...
        """
        ### YOUR CODE HERE ###
        Y_hat = self.forward(X) # do a forward pass to compute predicted y
       
        loss = self.loss.forward(Y, Y_hat)  # finding loss between targest and predictions
        
        return Y_hat, loss # returing predictied y and loss as a tuple